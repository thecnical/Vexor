...
def update():
    ...
    def _find_repo():
        # 1. Walk up from installed package
        try:
            import vexor as _v
            vf = Path(_v.__file__)
            for par in [vf.parent, *vf.parents]:
                if (par / ".git").exists(): return par
        except Exception: pass
        # 2. Common locations
        for c in [Path.home()/"Vexor", Path("/home/kali/Vexor"), Path("/root/Vexor"), Path("/opt/Vexor"), Path.cwd(), Path.cwd().parent]:
            if c.exists() and (c / ".git").exists(): return c
        return None

    try:
        repo = _find_repo()
        if repo:
            console.print(f"[bright_green]  Found repo:[/] {repo}")
            r = subprocess.run(["git", "pull", "origin", "main"], cwd=str(repo), capture_output=True, text=True)
            out = r.stdout.strip() or "Already up to date"
            if r.returncode == 0: console.print(f"[bright_green]  ✓ {out}[/]")
            else: console.print(f"[bright_yellow]  ! {r.stderr.strip()[:100]}[/]")
            cli = repo / "cli"
            if not cli.exists(): cli = repo
            flags = ["--quiet", "--break-system-packages"]
            r2 = subprocess.run([sys.executable, "-m", "pip", "install", "-e", "."] + flags, cwd=str(cli), capture_output=True, text=True)
            if r2.returncode == 0: console.print("[bright_green]  ✓ Updated! Restart terminal.[/]")
            else: console.print(f"[bright_red]  ✗ Reinstall failed:[/] {r2.stderr[:200]}")
        else:
            clone = Path.home() / "Vexor"
            console.print("[bright_yellow]  Repo not found. Cloning...[/]")
            if clone.exists(): subprocess.run(["git", "pull"], cwd=str(clone))
            else: subprocess.run(["git", "clone", "https://github.com/thecnical/Vexor", str(clone)])
            console.print(f"[bright_cyan]  Run: cd {clone} && ./install.sh[/]")
    except Exception as e:
        console.print(f"[bright_red]  Error:[/] {e}")
        console.print("[dim]  find / -name install.sh -path */Vexor/* 2>/dev/null[/]")
